...
ciphertext_pair = {}
for p1, p2 in test_pair:
    ciphertext_pair[strxor(p1, p2)] = [
        encrypt(p1),
        encrypt(p2)
    ]
...
for x in range(256):
    ...
    for p1, p2 in test_pair:
        ...
        p1_xor_p2 = strxor(p1, p2)
        ciphertext_a = ciphertext_pair[p1_xor_p2][0]
        ciphertext_b = ciphertext_pair[p1_xor_p2][1]
        
        round0_key = bytes([x]) + b'\x00'*(15)
        a_ = strxor(p1, round0_key)
        b_ = strxor(p2, round0_key)
        a_ = round1_to_round2(a_)
        b_ = round1_to_round2(b_)

        plain_diff = strxor(a_, b_)
...
for k in possible_key:
    ...
    if len(k) > max_len:
        max_len = len(k)
    index += 1
print("[+]", multi_pos*max_len, "possible key found")

# Inverse possible RoundKey to MasterKey using inv_key_expansion()
round_key = b''
for k in possible_key:
    round_key += bytes([k[0]])

master_key = inv_key_expansion(round_key, 3)
